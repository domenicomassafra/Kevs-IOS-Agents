...
The dashboard container intentionally binds `127.0.0.1` on the MiniPC. `deploy/setup-minipc.sh` configures Tailscale Serve when Tailscale is installed, so the UI/API gets tailnet HTTPS without exposing Fastify publicly. PostgreSQL listens on loopback plus the MiniPC Tailscale address generated by the setup script; it is not bound to the public/LAN wildcard.
