...
test('physical iOS lane has one default and one explicit disable switch', () => {
    ...
    assert.equal(physicalIosLaneEnabled({ PHONE_FARM_ENABLE_PHYSICAL_IOS: 'true' }), true);
    assert.equal(physicalIosLaneEnabled({ PHONE_FARM_ENABLE_PHYSICAL_IOS: 'false' }), false);
});
