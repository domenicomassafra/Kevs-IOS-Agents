...
function textSelector(value: unknown, command: string): string {
    if (typeof value === 'string' && value.trim()) return value.trim();
    if (value && typeof value === 'object' && !Array.isArray(value)) {
        const text = (value as { text?: unknown }).text;
        if (typeof text === 'string' && text.trim()) return text.trim();
    }
    throw new Error(`Maestro ${command} requires a text selector`);
}
...
function commandStep(command: MaestroCommand, appId: string): FlowStep[] {
    ...
    if (name === 'tapOn') return [{ action: 'tapText', text: textSelector(value, name), timeoutMs: 10_000 }];
    if (name === 'assertVisible') return [{ action: 'assertVisible', text: textSelector(value, name), timeoutMs: 1_000 }];
    if (name === 'inputText') {
    ...
    if (name === 'extendedWaitUntil') {
        ...
        if (wait.visible !== undefined) return [{ action: 'waitVisible', text: textSelector(wait.visible, name), timeoutMs: timeout(wait.timeout) ?? 10_000 }];
        if (wait.notVisible !== undefined) return [{ action: 'waitGone', text: textSelector(wait.notVisible, name), timeoutMs: timeout(wait.timeout) ?? 10_000 }];
